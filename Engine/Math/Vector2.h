...
namespace anvilHM
{
	struct Vector2
	{
		...
		//friend std::istream& >> (std::istream& stream, Vector2& vector)
	};
	...
	inline std::istream& operator >> (std::istream& stream, Vector2& v) 
	{
		...
		//{## , ##} how it looks
		std::string xs = line.substr(line.find("{")+1,  line.find(",")- line.find("{")-1 );//look for x points
		v.x = std::stof(xs);

		std::string ys = line.substr(line.find(",") + 1, line.find("}") - line.find(",") - 1);//lookin for y points
		v.y = std::stof(ys);
